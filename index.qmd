...
# Create comprehensive expected value plot
ggplot(ev_data, aes(x = Years)) +
  geom_line(aes(y = Expected_Value, color = "Expected Value (Arithmetic)"), linewidth = 1.5) +
  geom_line(aes(y = Geometric_Mean, color = "Geometric Mean"), linewidth = 1.5) +
  geom_hline(yintercept = initial_balance, linetype = "dashed", color = "red", alpha = 0.7) +
  geom_point(aes(y = Expected_Value), color = "#2E86AB", size = 3) +
  geom_point(aes(y = Geometric_Mean), color = "#F24236", size = 3) +
  scale_y_continuous(labels = scales::dollar_format(), 
                     trans = "log10",
                     breaks = c(100, 500, 1000, 2000, 5000, 10000, 20000)) +
  scale_color_manual(values = c("Expected Value (Arithmetic)" = "#2E86AB", 
                               "Geometric Mean" = "#F24236")) +
  labs(
    title = "Expected Value Analysis: Arithmetic vs. Geometric Mean",
    subtitle = "Multiplicative processes require geometric mean for accurate long-term predictions",
    x = "Number of Years (Coin Flips)",
    y = "Expected Balance (Log Scale)",
    color = "Calculation Method",
    caption = "Red dashed line shows initial balance ($1,000) | Log scale reveals the divergence"
  ) +
  theme_minimal() +
  theme(
